...
void loop() {
  ...
  const int sender_delay = 5000;
  static unsigned long sender_timer = 0;
  if (millis() > sender_timer + sender_delay) {
    Serial.println("------------Temps-------------");
    dog_temperature = _10klab::temperature::readSensorTemperature('D');
    Serial.println("Dog temperature = " + String(dog_temperature));
    fan_temperature = _10klab::temperature::readSensorTemperature('F');
  ...
  const int battery_read_delay = 10000;
  static unsigned long battery_measurement_timer = 0;
  if (millis() > battery_measurement_timer + battery_read_delay) {
    Serial.println("------------Bat-------------");
    battery_level = _10klab::battery::batteryLevel();
